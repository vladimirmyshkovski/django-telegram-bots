...
from .utils import (extract_data_from_command, get_bot_model,
...
receive_message = Signal(providing_args=['bot_id', 'chat_id',
                                         'type', 'text', 'message'])

receive_command = Signal(providing_args=['bot_id', 'chat_id', 'type',
                                         'command', 'payload', 'chat_id'])
...
def authentication_user(sender, **kwargs):
    if all(k in kwargs for k in ('command', 'payload', 'chat_id', 'type')):
        ...
        bot = Bot.objects.get(chat_id=bot_id)
